// Types matching spv_mobile.udl exactly — do not edit without updating both

export type SpvErrorKind = 'InvalidArgument' | 'Network' | 'Rpc' | 'Wallet' | 'Parse' | 'Internal';

export class SpvError extends Error {
  kind: SpvErrorKind;
  constructor(message: string, kind: SpvErrorKind) {
    super(message);
    this.name = 'SpvError';
    this.kind = kind;
  }
...
export interface Utxo {
  txid: string;
  index: number;        // u32
  value: number;        // u64 — safe for IRM amounts
  height: number;       // u64
  is_coinbase: boolean;
...
export interface AgreementParams {
  template_type: string;        // "simple-settlement" | "otc"
